...
let meta_list;
try {
    const response = await fetch('./packages/all_models.json');
    meta_list = await response.json();
} catch (error) {
...
let modalName = getUrlQuery('id') || 'haru_greeter_pro_jp';
let cubism4Model;
// 获取下拉菜单
...
meta_list.forEach(item => {
    ...
    if (modalName == item.name) {
        cubism4Model = "packages/" + item.relative_path;
        console.log("modalName:" + modalName + ",model_path:" + cubism4Model);
    }
});

dropdown.value = modalName;
// 添加事件监听器
...
// const cubism4Model = "/live2d/packages/haru_greeter_pro_jp/runtime/haru_greeter_t05.model3.json";
